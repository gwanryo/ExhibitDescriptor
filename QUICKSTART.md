...
┌─ Exhibit Descriptor ─────────────────────────┐
│ ① 전시 준비                                   │
│    Overlay 폰트 · 벽 판정 Layer · 언어 · 열림 방식│
│ ② 작품                                        │
...
> [!TIP]
> **VR 로 볼 전시라면** `① 전시 준비 > 열림 방식` 을 **`Proximity`** 로 바꿔 보세요.
> ⓘ 를 손으로 겨눌 필요 없이, 작품을 보면서 다가가기만 하면 설명이 펼쳐지고 지나가면 접힙니다.
> 본문이 길면 오른손 스틱 상하로 스크롤됩니다.
...
| VR 에서 ⓘ 를 못 누르겠다 | 열림 방식을 `Proximity` 로 (다가가면 저절로 열립니다) |
