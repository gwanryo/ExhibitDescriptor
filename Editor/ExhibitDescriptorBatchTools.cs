...
public static partial class ExhibitDescriptorTools
{
    ...
    ///
    /// <b>부호(+/-)는 추측하지 않습니다.</b> 런타임이 플레이어 머리 위치로 정합니다
    /// (<c>ExhibitInteractable._TickIcon</c> 의 <c>side</c>). 에디터가 부호를 가정하던 시절에는
    /// 로컬 -축이 관람자를 향한 작품에서 Panel 이 좌우 반전됐고, <c>Vector3.Angle ≈ 0</c> 검증은
    /// 180도 뒤집힌 Panel 을 통과시켜 이를 잡지 못했습니다. 글자가 읽히는 스크린샷만 검증입니다.
    /// </summary>
