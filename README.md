...
- 모서리에 걸려 **옆** 자리가 좁은 작품도 Validate 가 찾아 반대쪽을 권합니다. 방향을 자동으로
  뒤집지는 않습니다 — Right/Left 는 관람자 기준이라 걸어 다닐 때 판정이 흔들려 아이콘이 좌우로 튑니다

Panel 이 열려 있는 동안에는 **아이콘이 숨습니다.** Panel 의 안쪽 가장자리가 아이콘 자리에서
시작하므로, 아이콘을 남겨 두면 아이콘과 그 위의 "설명" 툴팁이 본문 첫 줄을 덮습니다. 아이콘이
꺼지면 Interact 대상이 아니라 툴팁도 함께 사라집니다. **닫기는 Panel 의 `×` 버튼으로 합니다.**
